function [Data, datName] = GTSRBdata_2(trNumeach, XuNo, Case)

% seed = 1;
% Case = 1;
load './Data/GTSRB/GTSRB_vector_2.mat'; % 德国交通标志数据集
[m,n] = size(X);
% X = X/255;
X = mapminmax(X',0,1)';
trPer = 0.9;
...
ui = 0;
for i = [0, 1, 2, 3, 4, 5, 7, 8, 9, 10, 15, 16, 17]
    if i ~= 2 && i ~= 5
        ui  = ui + 1;
        Xui = Xtrain(Ytrain == i, :);
        if i == 0
            DataU.Ux{ui} = Xui(randperm(size(Xui,1), XuNo(1)), :);  % traffic sign 0
        end
        if i == 1
            DataU.Ux{ui} = Xui(randperm(size(Xui,1), XuNo(2)), :);  % traffic sign 1
        end
        if i == 2
            DataU.Ux{ui} = Xui(randperm(size(Xui,1), XuNo(3)), :);  % traffic sign 2
        end
        if i == 3
            DataU.Ux{ui} = Xui(randperm(size(Xui,1), XuNo(4)), :);  % traffic sign 3
        end
        if i == 4
            DataU.Ux{ui} = Xui(randperm(size(Xui,1), XuNo(5)), :);  % traffic sign 4
        end
        if i == 5
            DataU.Ux{ui} = Xui(randperm(size(Xui,1), XuNo(6)), :);  % traffic sign 5
        end
        if i == 7
            DataU.Ux{ui} = Xui(randperm(size(Xui,1), XuNo(7)), :);  % traffic sign 7
        end
        if i == 8
            DataU.Ux{ui} = Xui(randperm(size(Xui,1), XuNo(8)), :);  % traffic sign 8
        end
        if i == 9
            DataU.Ux{ui} = Xui(randperm(size(Xui,1), XuNo(9)), :);  % traffic sign 9
        end
        if i == 10
            DataU.Ux{ui} = Xui(randperm(size(Xui,1), XuNo(10)), :);  % traffic sign 10
        end
        if i == 15
            DataU.Ux{ui} = Xui(randperm(size(Xui,1), XuNo(11)), :);  % traffic sign 15
        end
        if i == 16
            DataU.Ux{ui} = Xui(randperm(size(Xui,1), XuNo(12)), :);  % traffic sign 16
        end
        if i == 17
            DataU.Ux{ui} = Xui(randperm(size(Xui,1), XuNo(13)), :);  % traffic sign 17
        end
    end
end
