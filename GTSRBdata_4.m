function [Data, datName] = GTSRBdata_4(trNumeach, XuNo, Case)

% seed = 1;
% Case = 1;
load './Data/GTSRB/GTSRB_vector_4.mat'; % 德国交通标志数据集
[m,n] = size(X);
% X = X/255;
X = mapminmax(X',0,1)';
trPer = 0.9;
...
%         ui  = ui + 1;
        Xui = Xtrain(Ytrain == i, :);
        if i == 6
            DataU.Ux{i} = Xui(randperm(size(Xui,1), XuNo(1)), :);  % traffic sign 6
        end
        if i == 32
            DataU.Ux{i} = Xui(randperm(size(Xui,1), XuNo(2)), :);  % traffic sign 32
        end
        if i == 41
            DataU.Ux{i} = Xui(randperm(size(Xui,1), XuNo(3)), :);  % traffic sign 41
        end
        if i == 42
            DataU.Ux{i} = Xui(randperm(size(Xui,1), XuNo(4)), :);  % traffic sign 42
        end
    end
end
